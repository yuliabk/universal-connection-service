...
מצב: תשתית ההצפנה חוברה לממשקי StateStore. גם ה־witness חובר לאחסון מוצפן עצמאי. ה־runtime מחייב תצורת הצפנה ומסדים שהוכנו מראש. מעבר offline לנתונים קיימים מומש ב־SDK ובפקודת תחזוקה; בדיקות SQLite ו־PostgreSQL עברו ב־CI של 128227b (605 בדיקות ללא דילוגים). אין לטעון שהפעלת UCS הקיימת מצפינה metadata רק משום שמיגרציה 7 הותקנה.
