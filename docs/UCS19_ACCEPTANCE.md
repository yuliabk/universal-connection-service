...
1. **הצפנת metadata במנוחה.** תוצאות מוצפנות ב־AES-GCM; receipts, indexes, outbox, audit, approvals ו־witness עדיין כוללים metadata קריא בשכבת האחסון. TLS אינו מכסה דיסק או גיבויים. העבודה מתקדמת בהצפנה בשכבת האפליקציה בשני backends; חיבור מלא ל־runtime ול־witness והעברת נתונים קיימים טרם הושלמו. אין לסמן סעיף זה כהושלם על סמך דגל תצורה או הצפנת payload בלבד.
...
השלמת הצפנה בשכבת האפליקציה מתקדמת ב־[UCS19_METADATA_ENCRYPTION.md](UCS19_METADATA_ENCRYPTION.md). נוספו מאגר מסמכים טרנזקציוני מוצפן ומיגרציה 7, ומומש EncryptedStateStore עם ממשקי הבקרה וה־receipts. ה־runtime עדיין אינו מפעיל אותו וה־witness חובר למאגר מוצפן עצמאי. חיבור runtime ומעבר נתונים קיימים עדיין פתוחים. פער ההצפנה נשאר פתוח עד שילוב כל סוגי הנתונים ובדיקות coordinator/קריסה מול האחסון המוצפן.
